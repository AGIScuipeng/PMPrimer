...
from piece.piecebase import calc_conserve_continue, calc_std, calc_n_divide_average, calc_cv_value
...
class piecedesign() :
    ...
    def detect_conser_area(self, seqdict, threshold=0.95, minlen=15) :
        ...
        while posr < seqlen :
            ...
            if posr - posl + (1 if posr-posl == 0 else 0) >= minlen and ((mem[1]/len(seqdict.values()))/(1 if posr-posl == 0 else posr-posl)) >= threshold : posmem.append([posl, posr-1])
            posr += 1; posl = posr; mem = [0.0]*2
        self._base.baselog(BASE_DEBUG_LEVEL1, '\r比对后序列的所有保守区域探测完毕')
        return posmem

    #挖掘所有非保守区域
    def detect_non_conser_area(self, posmem, minlen=1) :
        posmem_len = len(posmem)
        return [[rang[1]+1, posmem[idx+1][0]-1] for idx, rang in enumerate(posmem) if idx != posmem_len-1 and posmem[idx+1][0] - rang[1] > minlen]

    #计算非保守区域的标准差，结果越接近0，多样性越高（当然要谨记排除掉大段连续的gap）
    #可能存在大多数100%的情况，尤其是使用中断法的时候，这种情况后续再解决
    def calc_non_conser_area_std(self, seqdict, posl, posr) :
        ...
        return round(calc_std(allstd) if posr-posl != 0 else allstd[0], 8)
        #return round(calc_n_divide_average(allstd), 8)
